...
function MyAssetDetail(props) {
    ...
    const setSale = async () => {
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            const transaction = await contract.setForSale(props.assetID,assetPrice)
        ...
        <div className="card m-4" style={{ maxWidth: "30rem" }}>
            <div className="card-body">
                ...
                <div className="card-text d-flex flex-column m-1 mb-3">
                    ...
                    {enableSale && <input type="number" className="form-control" id="Price"  onChange={e => setAssetPrice(e.target.value)} step="any" placeholder="Price" min="0.00" max="10000.00" step="0.01" />}
                </div>
                ...
                {props.avlToBuy&&<button className="btn btn-primary m-1" onClick={ApproveBuyer}>Approve Buyer</button>}
            </div>
        </div>
    </div>);
}
